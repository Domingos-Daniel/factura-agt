(async () => {
  ...
  try {
    console.log('Running AGT mock service tests (JS)...');
    ...
    assert(regRes.httpStatus === 200 && (regRes.response).requestID, 'registarFactura should return requestID');
    const requestID = (regRes.response).requestID;
    ...
    assert(obterRes1.httpStatus === 200 && obterRes1.response.statusResult?.requestID === requestID, 'obterEstado must include requestID');
    ...
    assert(obterRes2.httpStatus === 200 && obterRes2.response.statusResult?.documentStatusList, 'obterEstado after processing must include documentStatusList');
    ...
    assert(listarRes.httpStatus === 200 && (listarRes.response.documentResultCount || 0) >= 1, 'listarFacturas should return at least one document');
    ...
    assert(consultarRes.httpStatus === 200 && (consultarRes.response).documents?.length === 1, 'consultarFactura should return the document');
    ...
    assert(solicRes.httpStatus === 200 && (solicRes.response).resultCode === 1, 'solicitarSerie should succeed');
    ...
    assert(listarSeriesRes.httpStatus === 200 && (listarSeriesRes.response.seriesResultCount || 0) >= 1, 'listarSeries should list the created series');
    ...
    assert(validarRes.httpStatus === 200 && (validarRes.response).actionResultCode === 'C_OK', 'validarDocumento should return C_OK on first confirmation');
    ...
    assert(validarRes2.httpStatus === 200 && (validarRes2.response).actionResultCode !== 'C_OK', 'second validation should not be C_OK');
