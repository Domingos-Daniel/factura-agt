...
import { consultarFacturaRequest, zodToErrorList, normalizeSoftwareInfo } from '@/lib/schemas'
...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      consultarFacturaRequest.parse(payload)
    } catch (e: any) {
    ...
    if (privKey && payload?.taxRegistrationNumber && payload?.documentNo) {
      try {
        payload.jwsSignature = makeConsultarFacturaSignature(
    ...
    try {
      const res = await client.consultarFactura(payload)
