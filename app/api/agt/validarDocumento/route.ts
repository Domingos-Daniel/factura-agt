...
import { makeValidarDocumentoSignature } from '@/lib/server/jws'
import { validarDocumentoRequest, zodToErrorList, normalizeSoftwareInfo } from '@/lib/schemas'
...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      validarDocumentoRequest.parse(payload)
    } catch (e: any) {
    ...
    if (privKey && payload?.taxRegistrationNumber && payload?.documentNo) {
      try {
        payload.jwsSignature = makeValidarDocumentoSignature(
    ...
    try {
      const res = await client.validarDocumento(payload)
